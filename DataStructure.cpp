...
namespace {
  ...
  HEADER_C* makeHeader(char firstLetter) {
    ...
    for (int i = 0; i < ALPHA; ++i)
      header->ppItems[i] = nullptr;
    return header;
    }

  // Find header by firstLetter; keep track of prior for insertion
  HEADER_C* findHeader(HEADER_C* head, char firstLetter, HEADER_C** previousHeader) {
    ...
    if (current && current->cBegin == firstLetter)
      return current;
    return nullptr;
    }

  // Insert header in sorted order after prev (may be null for list-head)
  void insertHeaderSorted(HEADER_C*& head, HEADER_C* previous, HEADER_C* newHeader) {
    if (!previous) {
...
DataStructure::DataStructure(std::string Filename) : pStruct2(nullptr) {
  ...
  struct FileHeader {
    ...
    // Validate the header: check if read succeeded, magic number matches, and version is supported
    if (!inputFile || header.magic != 0x53324453u /*'S2DS'*/ || header.version != 1)
      throw std::runtime_error("Invalid file format");

    // Read each item based on the count in the header
    for (uint32_t itemIndex = 0; itemIndex < header.count; ++itemIndex) {
      // Read the length of the ID string (uint32_t, not including null terminator)
      uint32_t idLength = 0;
      ...
      // Safe to use &idString[0] since idLength > 0 and string is sized to idLength
      ...
      // This ensures the structure invariants are maintained (e.g., sorted headers, unique IDs)
      *this += newItem;
      }
  }
  ...
  while (currentHeader) {
    ...
    if (currentHeader->ppItems) {
      for (int bucketIndex = 0; bucketIndex < ALPHA; ++bucketIndex) {
        pointer_to_item currentItem = static_cast<pointer_to_item>(currentHeader->ppItems[bucketIndex]);
        while (currentItem) {
...
DataStructure::DataStructure(const DataStructure& Original)
  ...
  while (sourceHeader) {
    ...
    for (int bucketIndex = 0; bucketIndex < ALPHA; ++bucketIndex) {
      ...
      for (pointer_to_item sourceItem = static_cast<pointer_to_item>(sourceHeader->ppItems[bucketIndex]); sourceItem;
        sourceItem = sourceItem->pNext) {
...
int DataStructure::GetItemsNumber() {
  ...
  for (HEADER_C* header = pStruct2; header; header = header->pNext) {
    for (int bucketIndex = 0; bucketIndex < ALPHA; ++bucketIndex) {
      for (pointer_to_item item = static_cast<pointer_to_item>(header->ppItems[bucketIndex]); item;
        item = item->pNext)
        ++itemCount;
      }
    }
...
pointer_to_item DataStructure::GetItem(char* pID) {
  ...
  if (bucketIndex < 0)
    return nullptr;
  pointer_to_item currentItem = static_cast<pointer_to_item>(header->ppItems[bucketIndex]);
  while (currentItem) {
...
void DataStructure::operator+=(pointer_to_item p) {
  ...
  clonedItem->pNext = static_cast<pointer_to_item>(header->ppItems[bucketIndex]);
...
bool DataStructure::operator==(DataStructure& Other) {
  ...
  for (HEADER_C* header = pStruct2; header; header = header->pNext) {
    for (int bucketIndex = 0; bucketIndex < ALPHA; ++bucketIndex) {
      for (pointer_to_item itemA = static_cast<pointer_to_item>(header->ppItems[bucketIndex]); itemA;
        itemA = itemA->pNext) {
        pointer_to_item itemB = static_cast<pointer_to_item>(Other.GetItem(itemA->pID));
        if (!itemB)
          return false;
        ...
        if (itemA->pTime && itemB->pTime) {
          if (itemA->pTime->Hour != itemB->pTime->Hour ||
            itemA->pTime->Min != itemB->pTime->Min || itemA->pTime->Sec != itemB->pTime->Sec)
            return false;
          }
        }
      }
    ...
    for (HEADER_C* header = pStruct2; header; header = header->pNext) {
      for (int bucketIndex = 0; bucketIndex < ALPHA; ++bucketIndex) {
        for (pointer_to_item item = static_cast<pointer_to_item>(header->ppItems[bucketIndex]); item;
          ...
          outputFile.write(reinterpret_cast<const char*>(&idLength), sizeof(idLength));
          outputFile.write(item->pID, idLength);
          outputFile.write(reinterpret_cast<const char*>(&item->Code), sizeof(item->Code));
          char hasTimeFlag = item->pTime ? 1 : 0;
          outputFile.write(reinterpret_cast<const char*>(&hasTimeFlag), sizeof(hasTimeFlag));
          if (hasTimeFlag)
            outputFile.write(reinterpret_cast<const char*>(item->pTime), sizeof(TIME));
          }
        }
      }
  }
